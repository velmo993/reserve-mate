...
function save_booking_to_db($property_ids, $name, $email, $phone, $adults, $children, $pets, $start_date, $end_date, $total_cost, $paid_amount = 0, $payment_method, $client_request,  $admin = false) {
    // error_log(print_r(compact('property_ids', 'name', 'email', 'phone', 'adults', 'children', 'pets', 'start_date', 'end_date', 'total_cost', 'paid'), true));
...
function send_success_email_to_client($property_ids, $name, $email, $phone, $adults, $children, $pets, $start_date, $end_date, $total_cost, $paid_amount) {
        ...
        if (isset($message_settings['send_email_to_clients'])) {
            error_log("If message settings ok.");
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $emailErr = "Invalid email format";
            }
            //error_log(print_r($_GET, true));
            // error_log($_GET); Check for other variables here and add the data to email
            if(!isset($email)) {
                return;
            } else {
                ...
                $children_placeholder = (!empty($children) && $children != '0') ? "<li><strong>Gyerekek:</strong> $children</li>" : "";
                $pets_placeholder = (!empty($pets) && $pets != '0') ? "<li><strong>Háziállatok:</strong> $pets</li>" : "";
                ...
                if (!$email_sent) {
                    ...
                    $response = 'Email successfully to send to client';
                }
            }
        } else {
